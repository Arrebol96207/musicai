...
- 队列面板提供“本地曲库”标签页：浏览、搜索、收藏本地文件，一键加入队列或全部播放；FLAC 文件会解析真实时长。
- 播放时显示歌词卡：优先读取 FLAC 内嵌歌词，其次从 LRCLIB 在线搜索，支持逐行时间轴高亮滚动和折叠。
- 内置十段均衡器（8 种预设 + 自定义）、前置增益和实时频谱可视化，仅对本地音源生效，在线音源自动直通。
- 聊天输入框支持搜索历史回溯（上下键选择），侧边栏提供播放统计卡（总播放、累计收听时长、Top 5 常听）。
